...
int Board::RunMatch()
{
	...
	for (int i = 0; i < 64; i++)
	{
		...
		{
			Gem newGem = match[0]->Copy();
			newGem.flags = GemFlags::FLAME;

			matchColorCount[(int)match[0]->color]++;
			score.ScoreMatch(3);
			score.ScoreMatch(3);
			for (Gem* gem : match)
			{
...
/// <returns></returns>
Gem* Board::HopGems(std::vector<Gem*> run)
{
	int size = run.size();

	for (int i = 0; i < size; i++)
	{
		if (run[i]->rotated && run[i]->flags == GemFlags::NONE)
			return run[i];
	}
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (run[idx]->flags == GemFlags::NONE)
			return run[idx];
	}
